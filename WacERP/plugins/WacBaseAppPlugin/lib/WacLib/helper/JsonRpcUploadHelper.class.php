...
class JsonRpcUploadHelper {
	...
	if ($this->_chunk < 2 && file_exists($this->_config["targetDir"] . DIRECTORY_SEPARATOR . $this->_fileName)) {
		$ext = strrpos($this->_fileName, '.');
		$this->_fileName_a = substr($this->_fileName, 0, $ext);
		$this->_fileName_b = substr($this->_fileName, $ext);

		$count = 1;
		while (file_exists($this->_config["targetDir"] . DIRECTORY_SEPARATOR . $this->_fileName_a . '_' . $count . $this->_fileName_b))
			$count++;

		$this->_fileName = $this->_fileName_a . '_' . $count . $this->_fileName_b;
	}
	...
	if (is_dir($this->_config["targetDir"]) && ($dir = opendir($this->_config["targetDir"]))) {
		while (($file = readdir($dir)) !== false) {
			$filePath = $this->_config["targetDir"] . DIRECTORY_SEPARATOR . $file;
	...
	if (strpos($contentType, "multipart") !== false) {
		if (isset($_FILES['file']['tmp_name']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
			// Open temp file
			$out = fopen($this->_config["targetDir"] . DIRECTORY_SEPARATOR . $this->_fileName, $this->_chunk == 0 ? "wb" : "ab");
			if ($out) {
