...
class moduleListComponent extends WacModuleListComponent
{
    ...
    public function setupJqGridCols(){
        ...
        $editOptions = sprintf("{value:\"%s\"}", WacDataType::getInstance()->getIdCaptionHash(false, ':', ';', true ));

        $listCols = array();
        $listCols[] = $jqGridDataHelper->getCol(array("name"=>"id", "label"=>"id", "index"=>"id", "width"=>"50", "align"=>"left"));
        $listCols[] = $jqGridDataHelper->getCol(array("name"=>"name", "label"=>$i18n->__("Name"), "index"=>"name", "editable"=>"true", "width"=>"200", "align"=>"left"));
